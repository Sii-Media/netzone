import 'package:netzoon/domain/categories/entities/govermental_details.dart';
import 'package:netzoon/domain/categories/entities/governmental.dart';

List<Governmental> governmental = [
  const Governmental(
    name: 'مؤسسات دبي الحكومية',
    images: [
      'https://tec.gov.ae/documents/20124/973613/General+Directorate+of+Residency+and+Foreigners+Affairs+Dubai.jpg/d0a8e3b4-ff36-7ff8-c41f-6919291c270f?t=1609917504510',
      // 'https://mediaoffice.ae/-/media/feature/navigation/footer/government-logo/udotae.png',
    ],
    govermentalDetails: GovermentalDetails(
      govname: 'الادارة العامة للاقامة وشؤون الاجانب',
      name: 'الادارة العامة للاقامة وشؤون الاجانب',
      city: 'دبي',
      address: 'دبي',
      email: 'info@gmail.com',
      imgurl: 'imgurl',
      info:
          'توفر إقامة دبي خدمات أذونات الدخول وتصاريح الإقامة الذهبية،خدمات المنشأة،الجنسية وجوازات السفر،تأشيرة دبي،المخالفين والغرامات,البوابات الذكية.',
      mobile: '0808080',
      phone: '0808080',
      videourl: 'u-niGUDtUZE',
      link: 'https://www.gdrfad.gov.ae/ar',
    ),
  ),
  const Governmental(
    name: 'مؤسسات أبوظبي الحكومية',
    images: [
      // 'https://tec.gov.ae/documents/20124/973613/General+Directorate+of+Residency+and+Foreigners+Affairs+Dubai.jpg/d0a8e3b4-ff36-7ff8-c41f-6919291c270f?t=1609917504510',
      'https://cdn1-m.alittihad.ae/store/archive/image/2022/4/12/2a1e439b-1f36-48d0-9f3a-e00d1c612d55.jpg?width=1300',
    ],
    govermentalDetails: GovermentalDetails(
      govname: 'أكاديمية أبوظبي الحكومية',
      name: 'أكاديمية أبوظبي الحكومية',
      city: 'أبوظبي',
      address: 'أبوظبي',
      email: 'email',
      imgurl: 'imgurl',
      info: 'info@gmail.com',
      mobile: '0808080',
      phone: '0808080',
      videourl: 'u-niGUDtUZE',
      link: 'https://adsg.gov.ae/',
    ),
  ),
  const Governmental(
    name: 'مؤسسات الشارقة الحكومية',
    images: [
      // 'https://tec.gov.ae/documents/20124/973613/General+Directorate+of+Residency+and+Foreigners+Affairs+Dubai.jpg/d0a8e3b4-ff36-7ff8-c41f-6919291c270f?t=1609917504510',
      'https://sia.gov.ae/uploads/topics/16684933133393.jpg',
    ],
    govermentalDetails: GovermentalDetails(
      govname: 'حكومة الشارقة',
      name: 'حكومة الشارقة',
      city: 'الشارقة',
      address: 'الشارقة',
      email: 'email',
      imgurl: 'imgurl',
      info: 'الشارقة',
      mobile: '0808080',
      phone: '0808080',
      videourl: 'u-niGUDtUZE',
      link:
          'https://dhr.gov.ae/ar/e-services/services-for-sharjah-government-employee',
    ),
  ),
  const Governmental(
    name: 'مؤسسات عجمان الحكومية',
    images: [
      // 'https://tec.gov.ae/documents/20124/973613/General+Directorate+of+Residency+and+Foreigners+Affairs+Dubai.jpg/d0a8e3b4-ff36-7ff8-c41f-6919291c270f?t=1609917504510',
      'https://mediaoffice.ae/-/media/feature/navigation/footer/government-logo/udotae.png',
    ],
    govermentalDetails: GovermentalDetails(
      govname: 'مؤسسات عجمان الحكومية',
      name: 'مؤسسات عجمان الحكومية',
      city: 'عجمان',
      address: 'عجمان',
      email: 'email',
      imgurl: 'imgurl',
      info: 'info',
      mobile: '0808080',
      phone: '0808080',
      videourl: 'u-niGUDtUZE',
      link: '',
    ),
  ),
  const Governmental(
    name: 'مؤسسات رأس الخيمة الحكومية',
    images: [
      // 'https://tec.gov.ae/documents/20124/973613/General+Directorate+of+Residency+and+Foreigners+Affairs+Dubai.jpg/d0a8e3b4-ff36-7ff8-c41f-6919291c270f?t=1609917504510',
      'https://mediaoffice.ae/-/media/feature/navigation/footer/government-logo/udotae.png',
    ],
    govermentalDetails: GovermentalDetails(
      govname: 'مؤسسات رأس الخيمة الحكومية',
      name: 'مؤسسات رأس الخيمة الحكومية',
      city: 'رأس الخيمة',
      address: 'رأس الخيمة',
      email: 'email',
      imgurl: 'imgurl',
      info: 'info',
      mobile: '0808080',
      phone: '0808080',
      videourl: 'u-niGUDtUZE',
      link: '',
    ),
  ),
  const Governmental(
    name: 'مؤسسات أم القيوين الحكومية',
    images: [
      // 'https://tec.gov.ae/documents/20124/973613/General+Directorate+of+Residency+and+Foreigners+Affairs+Dubai.jpg/d0a8e3b4-ff36-7ff8-c41f-6919291c270f?t=1609917504510',
      'https://mediaoffice.ae/-/media/feature/navigation/footer/government-logo/udotae.png',
    ],
    govermentalDetails: GovermentalDetails(
      govname: 'مؤسسات أم القيوين الحكومية',
      name: 'مؤسسات أم القيوين الحكومية',
      city: 'أم القيوين',
      address: 'أم القيوين',
      email: 'email',
      imgurl: 'imgurl',
      info: 'info',
      mobile: '0808080',
      phone: '0808080',
      videourl: 'u-niGUDtUZE',
      link: '',
    ),
  ),
  const Governmental(
    name: 'مؤسسات الفجيرة الحكومية',
    images: [
      // 'https://tec.gov.ae/documents/20124/973613/General+Directorate+of+Residency+and+Foreigners+Affairs+Dubai.jpg/d0a8e3b4-ff36-7ff8-c41f-6919291c270f?t=1609917504510',
      'https://mediaoffice.ae/-/media/feature/navigation/footer/government-logo/udotae.png',
    ],
    govermentalDetails: GovermentalDetails(
      govname: 'مؤسسات الفجيرة الحكومية',
      name: 'مؤسسات الفجيرة الحكومية',
      city: 'الفجيرة',
      address: 'الفجيرة',
      email: 'email',
      imgurl: 'imgurl',
      info: 'info',
      mobile: '0808080',
      phone: '0808080',
      videourl: 'u-niGUDtUZE',
      link: '',
    ),
  ),
];
