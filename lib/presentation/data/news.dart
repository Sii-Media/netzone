// // import 'package:netzoon/domain/news/entities/news.dart';

// // List<News> news = [
// //   const News(
// //     title:
// //         'مدارس تقدم برامج خاصة خلال أسبوع رمضان لتقوية مهارات الطلبة وإعادة تهيئتهم',
// //     description:
// //         'فتحت مدارس الشارقة وعجمان، أمس، أبوابها لاستقبال الطلاب مع بدء دوام الفصل الثالث من العام الدراسي الجاري 2022/ 2023، حيث شهدت نسب إقبال متوسطة، لاسيما مع قرب إجازة عيد الفطر السعيد بنهاية الأسبوع الجاري.',
// //     imgUrl:
// //         'https://www.emaratalyoum.com/polopoly_fs/1.1739617.1681807932!/image/image.jpg',
// //     date: '27/04/2023',
// //     ownerName: 'BBC News',
// //     ownerImage:
// //         'https://is3-ssl.mzstatic.com/image/thumb/Purple112/v4/31/17/79/311779d6-bfe8-d8d5-4782-81bd4c5f01ea/AppIcon-0-0-1x_U007emarketing-0-0-0-7-0-0-sRGB-0-0-0-GLES2_U002c0-512MB-85-220-0-0.png/1200x630wa.png',
// //   ),
// //   const News(
// //     title: 'الذكاء الصناعي',
// //     description: 'الذكاء الصناعي واثره على العالم',
// //     imgUrl:
// //         'https://media.istockphoto.com/id/1311148884/vector/abstract-globe-background.jpg?s=612x612&w=0&k=20&c=9rVQfrUGNtR5Q0ygmuQ9jviVUfrnYHUHcfiwaH5-WFE=',
// //     date: '27/04/2023',
// //     ownerName: 'BBC News',
// //     ownerImage:
// //         'https://is3-ssl.mzstatic.com/image/thumb/Purple112/v4/31/17/79/311779d6-bfe8-d8d5-4782-81bd4c5f01ea/AppIcon-0-0-1x_U007emarketing-0-0-0-7-0-0-sRGB-0-0-0-GLES2_U002c0-512MB-85-220-0-0.png/1200x630wa.png',
// //   ),
// //   const News(
// //     title: 'الذكاء الصناعي',
// //     description: 'الذكاء الصناعي واثره على العالم',
// //     imgUrl:
// //         'https://static.vecteezy.com/system/resources/thumbnails/004/216/831/original/3d-world-news-background-loop-free-video.jpg',
// //     date: '27/04/2023',
// //     ownerName: 'BBC',
// //     ownerImage:
// //         'https://is3-ssl.mzstatic.com/image/thumb/Purple112/v4/31/17/79/311779d6-bfe8-d8d5-4782-81bd4c5f01ea/AppIcon-0-0-1x_U007emarketing-0-0-0-7-0-0-sRGB-0-0-0-GLES2_U002c0-512MB-85-220-0-0.png/1200x630wa.png',
// //   ),
// // ];
// import 'package:netzoon/domain/news/entities/news_info.dart';

// List<News> news = [
//   const News(
//     title:
//         'مدارس تقدم برامج خاصة خلال أسبوع رمضان لتقوية مهارات الطلبة وإعادة تهيئتهم',
//     description:
//         'فتحت مدارس الشارقة وعجمان، أمس، أبوابها لاستقبال الطلاب مع بدء دوام الفصل الثالث من العام الدراسي الجاري 2022/ 2023، حيث شهدت نسب إقبال متوسطة، لاسيما مع قرب إجازة عيد الفطر السعيد بنهاية الأسبوع الجاري.',
//     imgUrl:
//         'https://www.emaratalyoum.com/polopoly_fs/1.1739617.1681807932!/image/image.jpg',
//     ownerName: 'BBC',
//     ownerImage:
//         'https://is3-ssl.mzstatic.com/image/thumb/Purple112/v4/31/17/79/311779d6-bfe8-d8d5-4782-81bd4c5f01ea/AppIcon-0-0-1x_U007emarketing-0-0-0-7-0-0-sRGB-0-0-0-GLES2_U002c0-512MB-85-220-0-0.png/1200x630wa.png',
//   ),
//   const News(
//     title: 'الذكاء الصناعي',
//     description: 'الذكاء الصناعي واثره على العالم',
//     imgUrl:
//         'https://static.vecteezy.com/system/resources/thumbnails/004/216/831/original/3d-world-news-background-loop-free-video.jpg',
//     ownerName: 'BBC',
//     ownerImage:
//         'https://is3-ssl.mzstatic.com/image/thumb/Purple112/v4/31/17/79/311779d6-bfe8-d8d5-4782-81bd4c5f01ea/AppIcon-0-0-1x_U007emarketing-0-0-0-7-0-0-sRGB-0-0-0-GLES2_U002c0-512MB-85-220-0-0.png/1200x630wa.png',
//   ),
// ];
